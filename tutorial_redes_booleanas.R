...
net <- loadNetwork("/Volumes/KINGSTON/redes_booleanas_clase2025-2/practica_red_booleana_juguete/red_ejemplo.txt")
net
...
# En la tabla impresa, la primera columna denota el estado inicial, 
# la segunda columna contiene el estado después de la transición, 
# la primera columna contiene el número del atractor que se alcanza finalmente desde este estado, 
...
# extrae todos los estados de la tabla de transición que pertenecen a la cuenca de atracción del 
# atractor uno (es decir, cuyo número de atractor en la columna 3 es 1).
# Si está interesado en información sobre un único estado (en este caso: el estado con todos los genes a 1), 
# puede escribir


# obtener información sobre un solo estado (aquí: el estado con todos los genes a 1), podemos escribir
getStateSummary(attr, c(1,1,1))

# esta función de trazado se aplica por separado a cada cuenca de atracción, 
# y las cuencas se dibujan una al lado de la otra.
